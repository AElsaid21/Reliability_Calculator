...
def plot_interactive_charts(result_df: pd.DataFrame, has_related: bool, discrepancies: dict):
    ...
    buttons.append(
        dict(
            ...
            args=[
                {"visible": [True] * offset},
                {"title": "All Traces Displayed", "annotations": [dict(text="", showarrow=False)]}
            ]
        )
    )
    ...
    fig.update_layout(
        ...
        updatemenus=[
            go.layout.Updatemenu(
                buttons=buttons,
                x=0.5, y=1.1,  # update menu placed above the plot so it does not intersect with it
